...
var cSex = '';
var cAry = JSON.parse(window.localStorage.getItem('Contacts')) ? JSON.parse(window.localStorage.getItem('Contacts')) : [];//通过三目运算判断本地是否已有联系人信息数组，有则获取赋值，没有则赋值空数组
	...
	if (cName.value && cAge.value && cPhone.value) {
		...
		for (var i = 0; i < wUser.length; i++) {
			if (wUser[i].username == nowuser) {//遍历判断是否为当前登录账户的信息对象
				...
				window.localStorage.setItem('Contacts', JSON.stringify(cAry));//将联系人对象数组存入本地存储区
